...
int sock_send(sock_t *sock, void *buffer, size_t size, int socket) {
  ...
  worker->type.sender.buffer = buffer; worker->type.sender.size = size; worker->type.sender.socket = socket;
...
static void *sock_thread_listenner(void *arg) {
  ...
  while (1) {
    ...
    fd_set fds = worker->type.listenner.fds; struct timeval tv = { 5, 0 };
    ...
    for (int index = 0; index < FD_SETSIZE; index++) {
      if (FD_ISSET(index, &fds)) {
        if (worker->type.listenner.socket == index) {
          ...
          int c; struct sockaddr_in addr_client;
...
static void *sock_thread_reader(void *arg) {
  ...
  while (1) {
    ...
    while (true == connected) {
      ...
      /* Block until input arrives on one or more active sockets */
      fd_set fds = worker->type.reader.fds; struct timeval tv = { 5, 0 };
...
static void *sock_thread_sender(void *arg) {
  ...
  if (SOCK_SEND_ROUND_ROBIN == worker->type.sender.socket) {
    ...
    int socket = -1; int retry = 100; int loop = 0;
    ...
    sem_wait(&sock->clients.sem);
    for (int index = 0; index < FD_SETSIZE; index++) {
      if ((FD_ISSET(index, &sock->clients.fds)) && (worker->type.sender.size != send(index, worker->type.sender.buffer, worker->type.sender.size, MSG_NOSIGNAL))) {
        /* Unable to send data */
        FD_CLR(index, &sock->clients.fds);
        close(index);
      }
