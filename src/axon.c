...
axon_t *axon_create(char *type) {
  ...
  if ((axon->type == AXON_TYPE_SUB) || (axon->type == AXON_TYPE_PULL) || (axon->type == AXON_TYPE_REQ) || (axon->type == AXON_TYPE_REP)) {
 ...
 */
bool axon_is_connected(axon_t *axon, char *hostname, uint16_t port) {
...
int axon_vsend(axon_t *axon, int count, amp_type_e type1, void *value1, va_list params) {
  ...
  void *blob = NULL; size_t size = 0; char *str = NULL; int64_t bint = 0; cJSON *json = NULL; void *buffer = NULL;
  char str_id[32 + 1]; amp_msg_t **resp = NULL; int timeout = 0;
  char str_mq[64 + 1]; mqd_t mq;
  ...
  if (AXON_TYPE_REQ == axon->type) {
    ...
    snprintf(str_id, 32, "%d:%u", getpid(), axon->msg_id); axon->msg_id++;
...
amp_msg_t *axon_reply(axon_t *axon, int count, ...) {
  ...
  void *blob = NULL; size_t size = 0;
  char *str = NULL; int64_t bint = 0;
  cJSON *json = NULL;
    ...
    if (AXON_TYPE_REQ == axon->type) {
      ...
      if (NULL != axon->cb.message.fct) {
        ...
        if (NULL != rep) {
          ...
          /* Encode AMP message */
          void *buffer_rep = NULL; size_t size_rep = 0;
